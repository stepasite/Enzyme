; RUN: if [ %llvmver -ge 12 ] && [ %llvmver -lt 16 ]; then opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -mem2reg -instsimplify -simplifycfg -enzyme-coalese=1 -enzyme-postopt=1 -S | FileCheck %s; fi
; RUN: if [ %llvmver -ge 12 ]; then opt < %s %newLoadEnzyme -passes="enzyme,function(mem2reg,instsimplify,%simplifycfg)" -enzyme-coalese=1 -enzyme-postopt=1 -enzyme-preopt=false -S | FileCheck %s; fi

define double @square(double* noalias nocapture %arg, double* noalias nocapture %arg1) {
bb:
  br label %bb3
...
define dso_local double @dsquare(double* %arg, double* %arg1, double* %arg2, double* %arg3) {
bb:
  %i = tail call double @__enzyme_autodiff(i8* bitcast (double (double*, double*)* @square to i8*), double* %arg, double* %arg1, double* %arg2, double* %arg3)
  ret double %i
}

declare dso_local double @__enzyme_autodiff(i8*, double*, double*, double*, double*)

; CHECK: define dso_local double @dsquare(double* nocapture %arg, double* nocapture %arg1, double* nocapture %arg2, double* nocapture %arg3) 
; CHECK: bb:
; CHECK:   %0 = tail call {{(dereferenceable_or_null\(1600\) )?}}i8* @malloc(i64 1600){{( #[0-9]+)?}}, !noalias !{{[0-9]+}}, !enzyme_cache_alloc ![[ascope:.+]]
...
; CHECK: invertbb3.i:                                      ; preds = %invertbb3.i, %bb
; CHECK-NEXT:   %"iv'ac.0.i" = phi i64 [ 99, %bb ], [ %13, %invertbb3.i ]
...
; CHECK-NEXT:   br i1 %12, label %diffesquare.exit, label %invertbb3.i

; CHECK: diffesquare.exit:                                 ; preds = %invertbb3.i
