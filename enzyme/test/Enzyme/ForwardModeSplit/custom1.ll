if [ %llvmver -lt 16 ]; then %opt < %s %loadEnzyme -preserve-nvvm -enzyme -enzyme-preopt=false -mem2reg -simplifycfg -early-cse -S | FileCheck %s; fi
if [ %llvmver -ge 16 ]; then %opt < %s %newLoadEnzyme -passes="preserve-nvvm,enzyme,function(mem2reg,simplifycfg,early-cse)" -enzyme-preopt=false -S | FileCheck %s; fi
