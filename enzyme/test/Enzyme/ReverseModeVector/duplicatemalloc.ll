...
; RUN: if [ %llvmver -ge 14 ] && [ %llvmver -lt 16 ]; then %opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -inline -mem2reg -sroa -simplifycfg -instsimplify -early-cse -adce -dse -S | FileCheck %s -check-prefixes CALLOC,SHARED; fi
; RUN: if [ %llvmver -ge 16 ]; then %opt < %s %newLoadEnzyme -passes="enzyme,function(inline,mem2reg,sroa,simplifycfg,instsimplify,early-cse,adce,dse)" -enzyme-preopt=false -S | FileCheck %s -check-prefixes CALLOC,SHARED; fi
