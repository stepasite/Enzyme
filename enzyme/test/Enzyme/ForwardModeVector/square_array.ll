...
; RUN: %opt < %s %if [ %llvmver -lt 16 ]; then -enzyme -enzyme-preopt=false -mem2reg -simplifycfg -early-cse -S | FileCheck %s; fi
; RUN: %opt < %s %if [ %llvmver -ge 16 ]; then -passes="enzyme,function(mem2reg,simplifycfg,early-cse)" -enzyme-preopt=false -S | FileCheck %s; fi
