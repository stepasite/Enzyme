...
; RUN: if [ %llvmver -lt 16 ]; %opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -mem2reg -instsimplify -adce -correlated-propagation -simplifycfg -S | FileCheck %s; fi
; RUN: if [ %llvmver -ge 16 ]; %opt < %s %newLoadEnzyme -passes="enzyme,function(mem2reg,instsimplify,adce,correlated-propagation,simplifycfg)" -enzyme-preopt=false -S | FileCheck %s; fi
