...
; RUN: if [ %llvmver -lt 16 ]; then %opt < %s %loadEnzyme -enzyme -enzyme-preopt=false -inline -mem2reg -simplifycfg -adce -S | FileCheck %s; fi
; RUN: if [ %llvmver -ge 16 ]; then %opt < %s %newloadEnzyme -passes="enzyme,function(inline,mem2reg,simplifycfg,adce)" -enzyme-preopt=false -S | FileCheck %s; fi
